...
# from models.GCnet import ContextBlock,eca_layer,AugmentedConv
...
def se_densenet121(pretrained=True, is_strict=False, progress=True, **kwargs):
    ...
    if pretrained:
        ...
        state_dict = model_zoo.load_url(model_urls['densenet121'],progress=progress)
...
def se_densenet201(pretrained=True, progress=True, **kwargs):
    ...
    if pretrained:
        ...
        pattern = re.compile(
            r'^(.*denselayer\d+\.(?:norm|relu|conv))\.((?:[12])\.(?:weight|bias|running_mean|running_var))$')
        state_dict = model_zoo.load_url(model_urls['densenet201'],progress=progress)
        for key in list(state_dict.keys()):
            res = pattern.match(key)
            if res:
                new_key = res.group(1) + res.group(2)
...
class SEDenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        self.features = nn.Sequential(OrderedDict([
            ...
            # ("CALayer", ChannelAttention(num_init_features)),
            ("GCSALayer", ContextBlock(num_init_features, ratio=1. / 16., pooling_type='att')),
            # self.nc = ContextBlock(inplanes=self.inplanes, ratio=1. / 16., pooling_type='att')
            # ("SALayer", SpatialAttention()),
            ('pool0', nn.MaxPool2d(kernel_size=3, stride=2, padding=1)),
        ]))

        # Add SELayer at first convolution
        # self.features.add_module("SELayer_0a", SELayer(channel=num_init_features))
        # self.features.add_module("CALayer" , ChannelAttention(num_init_features))
        # self.features.add_module("SALayer" , SpatialAttention())
        ...
        for i, num_layers in enumerate(block_config):
            ...
            # self.features.add_module("CALayer_%da" % (i + 1), ChannelAttention(num_features))
            # self.features.add_module("SALayer_%da" % (i + 1), SpatialAttention())
            ...
            # self.features.add_module("GCSALayer_%da" % (i + 1),
            #                          ContextBlock(num_features, ratio=1. / 16., pooling_type='att'))

            if i != len(block_config) - 1:
                # Add a SELayer behind each transition block
                # self.features.add_module("SELayer_%db" % (i + 1), SELayer(channel=num_features))
                # self.features.add_module("CALayer_%db" % (i + 1), ChannelAttention(num_features))
                # self.features.add_module("SALayer_%db" % (i + 1), SpatialAttention())
        ...
        self.features.add_module("GCSALayer_%da" % (i + 1), ContextBlock(num_features, ratio=1. / 16., pooling_type='att'))
        # self.features.add_module("SALayer_%da" % (i + 1), SpatialAttention())
...
def test_se_densenet(pretrained=True):
    X = torch.Tensor(32, 3, 224, 224)

    if pretrained:
        model = se_densenet121(pretrained=pretrained)### , num_classes=30
